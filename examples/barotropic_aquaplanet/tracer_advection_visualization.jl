...
function TracerAdvectionVisualization!(tracer_advection_grid, θ₀, Nplots, Δt, plot_iteration_interval, prettytimes,
    ...
    if make_panelwise_visualization_animation_with_halos
        filename = joinpath(output_directory, "cubed_sphere_tracer_advection_panelwise_visualization_with_halos_θ")
        panelwise_visualization_animation(tracer_advection_grid, θ_time_series;
                                          with_halos=true, colorrange, colormap, framerate, filename)
    end

    if make_panelwise_visualization_animation
        filename = joinpath(output_directory, "cubed_sphere_tracer_advection_panelwise_visualization_θ")
        panelwise_visualization_animation(tracer_advection_grid, θ_time_series;
                                          colorrange, colormap, framerate, filename)
    end

    if make_geo_heatlatlon_visualization_animation
        filename = joinpath(output_directory, "cubed_sphere_tracer_advection_geo_heatlatlon_visualization_θ")
