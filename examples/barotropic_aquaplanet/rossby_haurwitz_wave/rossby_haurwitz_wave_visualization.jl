...
function RossbyHaurwitzWaveVisualization!(rossby_haurwitz_wave_grid, Nplots, Δt, plot_iteration_interval,
                                          ...
                                          output_directory::String = "rossby_haurwitz_wave_output",
                                          output_filename::String = "rossby_haurwitz_wave_output.jld2",
    ...
    for iPlot in 0:Nplots
        ...
        if make_geo_heatmap_visualization_plots
            fig = geo_heatmap_visualization(rossby_haurwitz_wave_grid, η_time_series[plot_iteration], "cc", title_η;
                                            geo_heatmap_type, ssh = true, colorrange = colorrange_η,
                                            colorbarlabel = "surface elevation", colormap)
            filename = joinpath(output_directory,
                                @sprintf("cubed_sphere_rossby_haurwitz_wave_geo_%s_visualization_η_%d.png",
                                         geo_heatmap_type, plot_iteration))
            save(filename, fig)
            fig = geo_heatmap_visualization(rossby_haurwitz_wave_grid, ζ_time_series[plot_iteration], "cc", title_ζ;
                                            geo_heatmap_type, colorrange = colorrange_ζ,
                                            colorbarlabel = "relative vorticity", colormap)
            filename = joinpath(output_directory,
                                @sprintf("cubed_sphere_rossby_haurwitz_wave_geo_%s_visualization_ζ_%d.png",
                                         geo_heatmap_type, plot_iteration))
            save(filename, fig)
        end
    end
    ...
    if make_geo_heatmap_visualization_animation
        filename = @sprintf("cubed_sphere_rossby_haurwitz_wave_geo_%s_visualization_η", geo_heatmap_type)
        geo_heatmap_visualization_animation(rossby_haurwitz_wave_grid, η_time_series, "cc", prettytimes,
                                            "Surface elevation";
                                            plot_frames, geo_heatmap_type, ssh = true, colorrange = colorrange_η,
                                            colormap, framerate, output_directory, filename)
        filename = @sprintf("cubed_sphere_rossby_haurwitz_wave_geo_%s_visualization_ζ", geo_heatmap_type)
        geo_heatmap_visualization_animation(rossby_haurwitz_wave_grid, ζ_time_series, "cc", prettytimes,
                                            "Relative vorticity";
