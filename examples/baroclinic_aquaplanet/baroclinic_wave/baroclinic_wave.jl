using CubedSphereAquaplanet.BaroclinicAquaplanet.BaroclinicWave
using Oceananigans
...
baroclinic_wave_parameters = BaroclinicWaveParameters()
baroclinic_wave_grid = BaroclinicWaveGrid(baroclinic_wave_parameters; arch)

Δt = 5minutes
# Estimate barotropic time step via the CFL condition using the Courant number, minimum grid spacing, and gravity wave 
# speed.
CourantNumber = 0.2
c = sqrt(baroclinic_wave_parameters.g * baroclinic_wave_parameters.Lz)
Δx = minimum_xspacing(baroclinic_wave_grid, Face(), Face(), Face())
Δy = minimum_yspacing(baroclinic_wave_grid, Face(), Face(), Face())
...
baroclinic_wave_simulation = BaroclinicWaveSimulation(arch;
                                                      parameters = baroclinic_wave_parameters,
                                                      grid = baroclinic_wave_grid,
                                                      substeps,
                                                      Δt,
                                                      stop_time,
                                                      Ntime,
                                                      ...
                                                      output_interval)
run!(baroclinic_wave_simulation)

include("baroclinic_wave_visualization.jl")
Nplots = Nframes
...
BaroclinicWaveVisualization!(baroclinic_wave_grid, Nplots, Δt, plot_iteration_interval, prettytimes, framerate;
                             iPlot_Start,
                             iPlot_Δ,
                             plot_states,
